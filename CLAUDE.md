...
- **Google Business Profile API: FULLY WORKING as of 2026-08-15** (case `5-6797000041158`). **Key finding: service accounts are not supported at all for this API** — confirmed directly by Google's GBP API support (ticket response, 2026-08-15): "service accounts and accounts that use access tokens they did not generate are not officially supported... we recommend you OAuth directly into one of the Google Accounts that manages the business listings." The `scripts/google-auth.mjs` JWT/service-account flow that works fine for GA4/GTM/Search Console does NOT work for GBP — calling it as the service account only ever returns that service account's own empty auto-created account, and accepting a Manager invitation via the API as a service account fails with an undocumented `FAILED_PRECONDITION` error (don't re-attempt the service-account path).
  ...
  - **To call the API**: exchange the stored `refresh_token` for a fresh access token via `POST https://oauth2.googleapis.com/token` (`grant_type: refresh_token`), same pattern every time — see `scratchpad/gbp-test2.mjs`/`gbp-test3.mjs` for a working example (scratchpad scripts are ephemeral per session, this pattern should be turned into a proper `scripts/gbp-auth.mjs` helper next time GBP work is needed, mirroring `scripts/google-auth.mjs`'s shape).
