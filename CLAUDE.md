...
- **Google Business Profile API: APPROVED as of 2026-08-12** (case `5-6797000041158`, after ~2.5 weeks pending). Confirmation email says: log into Google Cloud Console, search "Google Business Profile API," enable it for the project. **Not yet enabled/tested as of this note** — next session should enable it in Console, confirm `scripts/google-auth.mjs`'s service account has access (may need to be added as a manager on the GBP listing itself, separate from Cloud Console project permissions), and test a basic read call before trusting it for anything write-side. Once working, this replaces the fragile browser-automation workflow used throughout 2026-08 (managing Posts/Services/Photos/categories by clicking through business.google.com in the Browser pane) — that workflow had real, repeated friction: screenshots intermittently fail ("Browser pane is not displayed"), Google's SPA click handlers reject synthetic/untrusted JS clicks (only real mouse-simulated clicks from `computer` work), and there is no file-upload capability at all in this environment (native OS file-picker dialogs can't be driven), so photo uploads always required the user to select files manually. Three earlier attempts (case IDs 8-8141000041253, 4-7018000041535, 1-7940000041911) were dead ends from using the wrong request type ("Quota Increase" instead of "Application For Basic API Access") — irrelevant now that this is approved, kept here only so it's not repeated if a *different* Google API access request comes up.
