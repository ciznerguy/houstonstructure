...
- **Google Business Profile API: APPROVED and confirmed working as of 2026-08-12** (case `5-6797000041158`). The API itself is enabled and authenticates successfully via `scripts/google-auth.mjs` with scope `https://www.googleapis.com/auth/business.manage` — confirmed with a live 200 response from `GET https://mybusinessaccountmanagement.googleapis.com/v1/accounts`. **Blocking step remaining**: that call currently only returns the service account's own empty auto-created `PERSONAL` account (`claude-manager@eagle-497106.iam.gserviceaccount.com`), not the real business, because **the service account hasn't been added as a Manager on the actual GBP listing yet**. This is a separate permission layer from Cloud Console/IAM — it has to be granted by the user from inside the Business Profile itself (business.google.com → the listing → Business Profile settings → People and access → Add → email `claude-manager@eagle-497106.iam.gserviceaccount.com` → Manager or Owner role). Once that's done, re-run the same accounts.list call to confirm the real business shows up, then move on to the Business Information API (`mybusinessbusinessinformation.googleapis.com`) for locations/services/categories and the dedicated GBP Posts API for posts — these should replace the fragile browser-automation workflow used throughout 2026-08 (screenshots intermittently fail with "Browser pane is not displayed", Google's SPA rejects synthetic/untrusted JS clicks so only real mouse-simulated `computer` clicks work, and there is no file-upload capability in this environment at all, so photo uploads always required the user to pick files manually). Three earlier case IDs (8-8141000041253, 4-7018000041535, 1-7940000041911) were dead ends from using the wrong request type and are now irrelevant, kept only so the mistake isn't repeated for a *different* API request.
