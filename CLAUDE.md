...
- **Resolved 2026-09-23**: the old "service pages score ~78 mobile / LCP ~5.5s" issue is gone. A live PageSpeed run on `/services/foundation-repair` (mobile, Slow 4G, Moto G Power, Lighthouse 13.5) returned **Performance 95, FCP 1.0s, LCP 2.1s, CLS 0, TBT 230ms**. The long-form rewrite of the service pages plus the section images appear to have fixed it; no separate LCP blocker was ever found. Remaining headroom on that report, none of it urgent: "Improve image delivery" (137 KiB, the hero JPEGs are 200-400 KiB at 1600w and would be smaller as WebP), "Render-blocking requests" (220 ms), "Reduce unused JavaScript" (167 KiB, mostly GTM/GA4), and 3 long main-thread tasks.
- **Do not measure LCP/FCP inside the Claude browser pane.** Confirmed 2026-09-23: the pane pins `first-paint` to about 3.3s on every page regardless of content (the homepage and a service page both reported FP=FCP=LCP≈3,330ms while the load event fired at 1.5-2.4s). Resource timing and the identity of the LCP element are reliable there; the timings are not. Use the pagespeed.web.dev method below for real numbers.
...
- **Bing (set up 2026-09-26, matters because ChatGPT runs on Bing)**:
  - **Bing Places**: listing created by importing from the Google Business Profile at bingplaces.com (signed in with the Google account). The import carried the name, phone, website, five categories, hours, 15 photos and the description across verbatim, and **verification was skipped entirely** because the Google account was already verified. Always import rather than entering manually: it is faster and it guarantees NAP consistency. Two other Levi businesses (Levi Homes LLC, 7 Days Kitchens) came along in the same import and are out of scope. Listing was "Pending publish, ETA 7-12 days" on creation. Still empty: Social profiles (should hold `BUSINESS.googleMapsUrl`, and Facebook once its page name is fixed) and Email.
  - **Bing Webmaster Tools**: verified by importing from Google Search Console (one click, no separate verification), sitemap imported. `ciznerguy.com` was imported by accident and removed. URL Submission allows 100 URLs/day for immediate crawl, which Google has no equivalent of.
  - **IndexNow is wired up**: `node scripts/indexnow.mjs` pushes every URL in `out/sitemap.xml` to Bing/Yandex/Seznam/Naver, or specific paths if passed as arguments. Ownership is proved by `public/067f1beb0514f4d5f5ec714b72e6b2dd.txt`, which must stay in `public/`. **Build and deploy first, then run it** — the script checks the key file is live and exits if not. HTTP 200 or 202 both mean accepted. Google does not participate, so this never replaces Search Console.
  - **Hours aligned 2026-09-26**: all three now read Mon-Sat 8 AM - 8 PM, Sunday closed. `BUSINESS.hours` carries `opens`/`closes` in 24-hour form, and `Schema.tsx` derives `openingHoursSpecification` from those rather than hardcoding, so changing the hours in one place updates the footer and the JSON-LD together.
- **Finding demand before it reaches us** (added 2026-09-26). Two complementary scripts, both read-only and free:
  - `node scripts/gsc-emerging.mjs` compares three 28-day Search Console windows and prints queries that are brand new, queries that at least doubled, and queries sitting at position 8-20 where the upside per unit of work is highest. Bot already excluded.
  - `node scripts/autocomplete-harvest.mjs` scrapes Google's own suggest endpoint for our seed terms expanded a-z and with question prefixes. This is the only way to see demand we do **not** yet rank for, which is exactly what Search Console cannot show. Suggestion order is Google's popularity signal, so the printed rank means something.
  - A third throwaway, `scratchpad/gap-check.mjs`, diffs autocomplete questions against the questions our pages ask. Useful but noisy: it matches on shared content words, so treat "covered" as a hint and read the pairing before believing it.
- **Search Console: exclude the "foundation engineer" bot** (identified 2026-09-21). A third-party rank tracker (not the user's) queries "foundation engineer" every Monday, desktop only, landing on `/` at position 1.0 with zero clicks: ~40% of all impressions (1,174 of 2,910 in 90 days). Always subtract it from totals/pages/dates before reporting. Do NOT use a `query notEquals` filter for this: any query-dimension filter makes GSC drop the anonymized long-tail rows, which hold most real clicks (23 → 1). Pull unfiltered, pull the bot's own rows with `query equals "foundation engineer"`, and subtract — `scratchpad/gsc-positions-check.mjs` does this. With the bot removed, Mondays are normal.
...
- **Google Business Profile API: FULLY WORKING as of 2026-08-15** (case `5-6797000041158`). **Key finding: service accounts are not supported at all for this API** — confirmed directly by Google's GBP API support (ticket response, 2026-08-15): "service accounts and accounts that use access tokens they did not generate are not officially supported... we recommend you OAuth directly into one of the Google Accounts that manages the business listings." The `scripts/google-auth.mjs` JWT/service-account flow that works fine for GA4/GTM/Search Console does NOT work for GBP — calling it as the service account only ever returns that service account's own empty auto-created account, and accepting a Manager invitation via the API as a service account fails with an undocumented `FAILED_PRECONDITION` error (don't re-attempt the service-account path).
  - **Working setup**: a separate OAuth 2.0 Client (type "Desktop app", named "GBP CLI Access") was created under the `eagle-497106` project's own Google Auth Platform (External, Testing mode, `ciznerguy@gmail.com` added as a test user — no Google app-verification review needed since it's single-user). Credentials live in `.secrets/gbp-oauth-client.json` (`client_id`/`client_secret`) and `.secrets/gbp-oauth-token.json` (long-lived `refresh_token`, obtained once via the installed-app OAuth flow authenticating as `ciznerguy@gmail.com`, who is Owner on the real GBP listing — no invitation/acceptance dance needed this way). Both files are gitignored under `.secrets/`.
  - **To call the API**: `import { getGbpAccessToken, GBP_LOCATION } from "../scripts/gbp-auth.mjs"` (added 2026-09-21, mirrors `scripts/google-auth.mjs`). It exchanges the stored `refresh_token` for an access token.
  - **Refresh token expiry (bit us 2026-09-21)**: while the OAuth app is in **Testing** mode, Google expires refresh tokens after 7 days, so the API returns `invalid_grant` on every call after that. Fix: run `node scratchpad/gbp-oauth-flow.mjs`, give the user the printed URL, they sign in as `ciznerguy@gmail.com` and consent, the token file is rewritten (the process may exit with a libuv assertion on Windows after printing "Success" — the token is saved regardless). Permanent fix, not yet done: user publishes the app (Cloud Console → `eagle-497106` → OAuth consent screen → Publish App); production refresh tokens don't expire.
  - **GBP Performance API works**: `scratchpad/gbp-performance.mjs` pulls search/maps impressions, website clicks, calls, directions (daily, 30d vs previous 30d) and the monthly search-keyword list. Keyword counts under 15 come back as `threshold: 15`, not a number. This is the only source for Local Pack visibility; Search Console does not report local results.
  - **Confirmed working calls**: `GET https://mybusinessaccountmanagement.googleapis.com/v1/accounts` → returns Guy's personal account (`accounts/117126009752898242838`). `GET https://mybusinessbusinessinformation.googleapis.com/v1/accounts/117126009752898242838/locations?readMask=...` → lists all of Guy's business locations, including **`locations/7994953544673911814` = Levi's Houston Structural Repairs, Engineering & Consulting** (the ID to use for this project going forward — save an API call by hardcoding it rather than re-listing every time).
  - **Next step, not yet done**: use the Business Information API (read/update location details, categories, services) and the dedicated GBP Posts API to replace the fragile browser-automation workflow used throughout 2026-08 for managing Posts/Services/Photos (screenshots intermittently failed with "Browser pane is not displayed", Google's SPA rejected synthetic/untrusted JS clicks so only real mouse-simulated `computer` clicks worked, and there was no file-upload capability at all in that environment). Photo uploads specifically may still need the Business Information API's media endpoints tested before fully replacing the manual-upload workflow.
  - Three early case IDs (8-8141000041253, 4-7018000041535, 1-7940000041911) were dead ends from using the wrong request type when first requesting API access; irrelevant now, kept only so that mistake isn't repeated for a *different* API request.
